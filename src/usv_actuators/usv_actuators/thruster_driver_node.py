...
class ThrusterDriverNode(Node):
    ...
    def control_loop(self):
        ...
        # 명령 단절은 ramp와 방향 전환 대기보다 우선한다.
        if self.last_cmd_time is None or now - self.last_cmd_time >= CMD_TIMEOUT:
            if not self.timed_out:
                self.get_logger().warning('/cmd_vel timeout -> neutral')
            self.timed_out = True
            self.target_left = self.target_right = NEUTRAL_PWM
            self.cur_left = self.cur_right = NEUTRAL_PWM
            self.hold_until = 0.0
        elif (self.sign(self.target_left) * self.sign(self.cur_left) < 0 or
        ...
        try:
            ...
            # notify는 응답을 기다리지 않는다. SEND는 MCU 적용 확인이 아니다.
            self.get_logger().info(
                f'PWM linear={self.linear:.3f} angular={self.angular:.3f} '
                f'timeout={self.timed_out} '
                f'TARGET L={self.target_left} R={self.target_right} '
                f'SEND L={self.cur_left} R={self.cur_right} us (unconfirmed)',
...
def main(args=None):
    ...
    try:
        ...
        try:
            ...
            node.get_logger().info('Shutdown neutral requested (unconfirmed)')
        except Exception as error:
            node.get_logger().error(f'Shutdown neutral request failed: {error}')
        node.destroy_node()
        rclpy.shutdown()
